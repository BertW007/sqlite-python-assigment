...
class Gui:
	def __init__(self):
		...
		self.id_label = tkinter.Label(self.top_frame, text='To change configurations: ').grid(row=0,column=0, pady=10)
		self.id_entry = tkinter.Button(self.top_frame, text='Change', command=self.change_config).grid(row=0,column=1, pady=10)

		#Sets the label\entry widgets - NAME
		self.name_label = tkinter.Label(self.top_frame, text='To open the graph and filter: ').grid(row=1,column=0, pady=10)
		self.name_entry = tkinter.Button(self.top_frame, text='Show', command = self.show_graph).grid(row=1,column=1, pady=10)
